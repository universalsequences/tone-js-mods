 ...
 */
Tone.Sampler.prototype.triggerAttack = function(notes, time, velocity, start=0, onended, output=this.output){
	...
	for (var i = 0; i < notes.length; i++){ 
		...
		source.start(time, start, undefined, velocity);
 ...
 * @return {Tone.Sampler}	this
 */
Tone.Sampler.prototype.triggerRelease = function(notes, time, releaseTime){
	if (!Array.isArray(notes)){
		notes = [notes];
	}
	for (var i = 0; i < notes.length; i++){  
		...
		if (this._activeSources[midi] && this._activeSources[midi].length){
			time = this.toSeconds(time);
                    let sources = this._activeSources[midi];
			var source = sources[sources.length - 1];
		    source.stop(time, releaseTime);
                    /*
                            if (this._activeSources &&
                                this._activeSources[midi] && 
                                this._activeSources[midi].length > 0) {
                                this._activeSources[midi].shift();
                            }
                        }, time * 1000);
                        */
		}
 ...
 * @return {Tone.Sampler}          this
 */
Tone.Sampler.prototype.triggerAttackRelease = function(notes, duration, time, velocity, start=0, onended, output){
    time = this.toSeconds(time);
    let source = this.triggerAttack(notes, time, velocity, start, onended, output);
    /*
    ...
    source.stop(time + this.toSeconds(duration));
    return source;
};
